...
class GUI():
    ...
    def create_gui_elements(self):
        ...
        self.tab_fileman = tkinter.Frame(self.tab_control, border=5) #вкладка ФАЙЛЫ
        self.tab_taskman = tkinter.Frame(self.tab_control, border=5) #вкладка ДИСПЕТЧЕР ЗАДАЧ
        self.tab_python = tkinter.Frame(self.tab_control, border=5) #вкладка PYTHON
        #
        ...
        self.tab_control.add(self.tab_fileman, text='файлы')
        self.tab_control.add(self.tab_taskman, text='процессы')
        self.tab_control.add(self.tab_python, text='python')
        ...
        self.ipports_main_frame = tkinter.Frame(self.tab_main, bg='gray', padx=1, pady=1)
        self.ipports_main_frame.pack(anchor='n', fill='both')
        self.ipports_frame = tkinter.Frame(self.ipports_main_frame, pady=2, padx=2)
        self.ipports_frame.pack(anchor='n', fill='both')
        ...
        #расставляю поля ввода данных для подключения
        ...
        #кнопки подключения
        self.button_connect = tkinter.Button(self.ipports_frame, text='подключиться', command=self.try_connect)
        self.button_autoconnect = tkinter.Button(self.ipports_frame, text='автоподключение по ip')
        #расставляю кнопки подключения
        ...
        #ПРАВАЯ КОЛОНКА
        self.window_view_main_frame = tkinter.Frame(self.tab_main, bg='gray', border=1)
        self.window_view_main_frame.pack(side='right',anchor='nw', pady=2, fill='both')
        self.window_view_frame = tkinter.Frame(self.window_view_main_frame, border=5)
        self.window_view_frame.pack(side='right',anchor='ne',fill='both')
        #настройки вида окна
        self.window_view_label = tkinter.Label(self.window_view_frame, text='настройки окна', relief='ridge', bg='lightgray')
        self.button_topmost = tkinter.Button(self.window_view_frame, text='поверх всех окон', bg='pink', command=self.topmost)
        self.alpha_button = tkinter.Button(self.window_view_frame, text='эффект прозрачности', bg='pink',command=self.set_alpha)
        #расположение кнопок
        self.button_topmost.pack(side='bottom', fill='both')
        self.alpha_button.pack(side='bottom', fill='both')
        self.window_view_label.pack(side='bottom', fill='both')
        #сохранение/загрузка профиля
        self.profile_text = tkinter.Label(self.window_view_frame, text='рабочий профиль', relief='ridge', bg='lightgray')
        self.save_button = tkinter.Button(self.window_view_frame, text='сохранить как...')
        self.load_button = tkinter.Button(self.window_view_frame, text='загрузить')        
        #расположение кнопок профиля
        self.profile_text.pack(fill='both')
        self.save_button.pack(fill='both')
        self.load_button.pack(fill='both')


        #ЛЕВАЯ БОЛЬШАЯ ПАНЕЛЬ НАСТРОЕК
        '''тут будут находится различные переключатели настроек'''
        self.settings_main_frame = tkinter.Frame(self.tab_main,  border=1)
        self.settings_main_frame.pack(side='top',anchor='nw', pady=2, fill='both')
        self.save_to_file = tkinter.Checkbutton(self.settings_main_frame, text='сохранять вывод консоли в файл', command=self.try_connect)
        self.save_to_file.pack(side='left')
        ...
        for task in range(9):
            ...
            self.tasks[-1].execute_now_button.pack(fill='both')
            self.tasks[-1].frame.pack(fill='both')
        ...
        #====ЭЛЕМЕНТЫ_НА_ВКЛАДКЕ_ФАЙЛЫ=====#

        #====ЭЛЕМЕНТЫ_НА_ВКЛАДКЕ_ПРОЦЕССЫ==#

        #====ЭЛЕМЕНТЫ_НА_ВКЛАДКЕ_PYTHON====#
